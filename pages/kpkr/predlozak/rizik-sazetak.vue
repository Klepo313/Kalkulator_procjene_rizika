      ...
      <h1>
        ...
        <span class="h1-container">
          <span
            ...
            <div v-if="item.value">
              {{ item.value }}
              <span v-if="index !== filteredItems.length - 1">{{
                item.suffix
              }}</span>
            </div>
          </span>
        ...
        <TabPanels class="tab-panel">
          <TabPanel value="0" class="tab-panel" ref="tabPanelRef">
            <div ref="rizikSazetakRef" class="sazetak-container">
              <Djelatnost
                v-if="vrstaIzracuna === 'Djelatnost'"
                :aiz_id="idIzracuna"
                :tip="'RZ'"
                class="rizik-sazetak" />
              <TablicaRizika
                v-else-if="vrstaIzracuna === 'Imovina'"
                :aiz_id="idIzracuna"
                :tip="'RZ'" />
    ...
    <footer>
      ...
      <div class="actionButtons">
        ...
        <button
          ...
          :disabled="!structuredDataBezMjera || !structuredDataSaMjerama"
    ...
    <div class="popup-expand" v-if="isPopupOpen">
      <div class="expand-content">
        ...
        <transition :name="slideDirection">
          <div v-show="tabs.length > 0" class="content-tabs">
            <!-- Dodano v-show ovdje -->
            <div
              ...
              class="tab">
              <Djelatnost
                v-show="vrstaIzracuna == 'Djelatnost' && tab.tip === 'RZ'"
                :aiz_id="idIzracuna"
                :tip="tab.tip"
                class="rizik-sazetak" />
              <TablicaRizika
                v-show="vrstaIzracuna == 'Imovina' && tab.tip === 'RZ'"
                :aiz_id="idIzracuna"
                :tip="tab.tip" />
              <Djelatnost
                v-show="vrstaIzracuna == 'Djelatnost' && tab.tip === 'KR'"
                :aiz_id="idIzracuna"
                :tip="tab.tip"
                class="rizik-sazetak" />
              <TablicaRizika
                v-show="vrstaIzracuna == 'Imovina' && tab.tip === 'KR'"
                :aiz_id="idIzracuna"
                :tip="tab.tip" />
            </div>
          </div>
...
const structuredStore = useStructuredGridDataStore();

const idIzracuna = computed(() => props.aizId);
if (!idIzracuna.value) {
  idIzracuna.value = cardStore.cardId || getIdFromUrl();
}
...
//
onMounted(async () => {
  // reset & fetch adapt measures
  if (idIzracuna.value && !adaptStore.odabrane_mjere?.length) {
    await adaptStore.fetchMetrictypes(idIzracuna.value);
  }

  // adjust summary panel width
  const panel = tabPanelRef.value;
  if (panel?.clientWidth < 1100) {
    maxSazetakWidth.value = "1400px";
  }
});

//
// 4) Computed wrappers & H1 items
//
const datum = computed(() => opciStore.izracun?.aiz_datum);
const brojIzracuna = computed(() => cardStore.broj);
const datumRaw = computed(() => opciStore.izracun?.aiz_datum);
const formatiraniDatum = computed(() =>
  datumRaw.value ? formatDateToDMY(datumRaw.value, ".") : null
);

// build the pieces for the H1 line
const vrstaIzracuna = computed(() => opciStore.izracun?.tvz_naziv || "");
console.log(vrstaIzracuna.value);
...
const katCestica = computed(() =>
  opciStore.izracun?.kcs_sif ? opciStore.izracun.kcs_sif : null
);
const vrstaImovine = computed(() => opciStore.izracun?.tvo_naziv || "");
const vrstaImovineId = computed(() => opciStore.izracun?.tvo_id || "");
const djelatnost = computed(() =>
  opciStore.izracun?.djl_naziv ? opciStore.izracun.djl_naziv : null
);
const djelatnostSifra = computed(() => opciStore.izracun.djl_sif);
const skupinaDjelatnosti = computed(
  () => opciStore.izracun?.djl_naziv_sk || ""
);
...
const displayItems = computed(() => [
  {
    value:
      katOpcinaSifra.value && katOpcina.value
        ? `${katOpcinaSifra.value} - ${katOpcina.value}`
        : "",
    suffix: ",",
  },
...
const filteredItems = computed(() =>
  displayItems.value.filter((it) => it.value)
);

//
// 5) Structured & adapt measures
//
const structuredDataBezMjera = computed(
  () => structuredStore.structuredDataBezMjera
);
const structuredDataSaMjerama = computed(
  () => structuredStore.structuredDataSaMjerama
);
const adaptMjere = computed(() => adaptStore.odabrane_mjere);

//
// 6) Tabs & Popup control
//
const tabs = ref([{ tip: "RZ" }, { tip: "KR" }]);
...
function switchTab(direction: number) {
  ...
  currentTab.value =
    (currentTab.value + direction + tabs.value.length) % tabs.value.length;
...
// 7) Layout refs
//
const tabPanelRef = ref<null>(null);
const maxSazetakWidth = ref("100%");
watch(
  tabPanelRef,
  (el) => {
    if (el?.clientWidth < 1100) maxSazetakWidth.value = "1400px";
  },
  { immediate: true }
);

//
// 8) Navigation & new calculation
//
function navigateWithParameter(path: string, key: string, val: string | null) {
  return val ? navigateTo({ path, query: { [key]: val } }) : navigateTo(path);
}

function noviIzracun() {
...
const doubleCols = letters.flatMap((c) => letters.map((d) => c + d));
const allColumns = [...letters, ...doubleCols];

function mapDataToCellRanges(
  structuredData: Record<string, any[]>,
  cellRanges: Record<string, { range: string }>
) {
  const cellData: Array<{ cell: string; value: any }> = [];
  for (const [pos, { range }] of Object.entries(cellRanges)) {
    const [start, end] = range.split(":");
    const startRow = +start.replace(/\D/g, "");
    const endRow = +end.replace(/\D/g, "");
    const colIdx = allColumns.indexOf(start.replace(/\d/g, ""));
    const rows = structuredData[pos] || [];
    for (let r = startRow; r <= endRow; r++) {
      ...
      [
        ...
        if (value != null) {
          cellData.push({ cell: `${allColumns[colIdx + idx]}${r}`, value });
        }
      });
    }
  }
...
const downloadRizikSazetak = async () => {
  try {
    ...
    const formattedString = filteredItems.value
      .map((item, index) => {
        return `${item.value}${
          index !== filteredItems.value.length - 1 ? item.suffix + " " : ""
        }`;
      })
      .join("");
    ...
    const bezMjeraWorksheet = workbook.getWorksheet(
      "Izračun bez mjera prilagodbe"
    );
    const saMjeramaWorksheet = workbook.getWorksheet(
      "Izračun s mjerama prilagodbe"
    );
    ...
    const opciPodaci = [
      ...
      {
        value:
          djelatnostSifra.value && djelatnost.value
            ? `${djelatnostSifra.value} - ${djelatnost.value}`
            : "/",
        row: 24,
      },
    ...
    if (vrstaIzracuna.value == "Djelatnost") {
      bezMjeraPopis = mapDataToCellRanges(
        structuredDataBezMjera.value,
        djelatnostCellRanges
      );
      saMjeramaPopis = mapDataToCellRanges(
        structuredDataSaMjerama.value,
        djelatnostCellRanges
      );
    ...
    bezMjeraPopis.forEach(({ cell, value }) => {
      ...
      const parsedValue =
        isNaN(value) || value === null ? null : parseInt(value); // Provjeri je li vrijednost broj i nije null
    ...
    const vrstaIzracunaShort =
      vrstaIzracuna.value === "Imovina"
        ? "IM"
        : vrstaIzracuna.value === "Djelatnost"
        ? "DJ"
        : "";

    let vrstaObjektaIdShort = "";
    if (vrstaImovineId.value === 50) vrstaObjektaIdShort = "PO";
    else if (vrstaImovineId.value === 51) vrstaObjektaIdShort = "GZ";
    else if (vrstaImovineId.value === 52) vrstaObjektaIdShort = "PZ";

    const katCesticaClean = katCestica.value
      ? katCestica.value.replace("/", "-")
      : "";
...
/**
 * Mapiranje structured podataka na ćelijske adrese
 */
// function mapDataToCellRanges(
//   structuredData: Record<string, any[]>,
//   ranges: Record<string,{range:string}>
// ): Array<{cell:string,value:any}> {
//   const cols = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('').flatMap(c=> 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('').map(d=>c+d))
//   const res: Array<{cell:string,value:any}> = []
//   for (const [pos,{range}] of Object.entries(ranges)) {
//     const [st,en] = range.split(':')
//     const sr = +st.replace(/\D/g,''), er = +en.replace(/\D/g,'')
//     const ci = cols.indexOf(st.replace(/\d/g,''))
//     const rows = structuredData[pos]||[]
//     for (let r=sr; r<=er; r++) {
//       const rowData = rows[r-sr]
//       if (!rowData) continue
//       ['history','p0_4_5','p1_4_5','p2_4_5','p0_8_5','p1_8_5','p2_8_5'].forEach((k,idx)=>{
//         const v = (rowData as any)[k]
//         if (v!=null) res.push({ cell: `${cols[ci+idx]}${r}`, value: v })
//       })
//     }
//   }
//   return res
