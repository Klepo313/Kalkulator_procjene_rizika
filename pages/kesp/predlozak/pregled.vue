    ...
    <main>
      ...
      <div class="main-content">
          ...
          <div class="razdoblje">
            ...
            <div>
              <div v-if="opis" class="opisnap">
                <label for="opis"> Naziv </label>
                <InputText
                  id="opis"
                  v-model="opis"
                  type="text"
                  placeholder="Naziv nije unešen"
                  readonly
                />
              </div>
              <div>
              ...
              <div v-if="napomena" class="opisnap">
                ...
                <Textarea
                  id="napomena"
                  v-model="napomena"
                  placeholder="Napomena nije unešena"
                  readonly
                />
              </div>
            </div>
        ...
        <section>
          ...
          <div class="datatable">
            <DataTable
              v-model:selection="odabranaGrupaEmisije"
            ...
            >
              ...
              <template #footer>
                <div class="flex justify-end font-bold w-full mt-4">
                  Ukupno:
                  <strong>{{
                    formatNumber(vehicleStore.ukupnaEmisija / 1000, 3)
                  }}</strong>
                  eCO<sub>2</sub> t/god
                </div>
              </template>
            ...
            <Dialog
              v-model:visible="dialogEmisijaVisible"
              :style="{ width: '75vw' }"
              maximizable
              modal
              :content-style="{ height: 'auto' }"
              @hide="odabranaGrupaEmisije = null"
            >
              <template #header>
                <span style="font-size: 1.1rem; font-weight: 600">
                  <font-awesome-icon
                    :icon="getVehicleIcon(odabranaGrupaEmisije?.uge_naziv)"
                  />
                  {{ odabranaGrupaEmisije?.uge_naziv }}
                </span>
              </template>
              <DataTable
                :value="energentiPotrosnja"
                table-style="min-width: 50rem"
                show-gridlines
              >
                <template #empty>
                ...
                <Column
                  field="energent"
                  :header="odabranaGrupaEmisije?.uge_lab_uvg"
                >
                  <template #body="slotProps">
                    <Tag
                      :value="slotProps.data.energent"
                      :class="[
                        {
                          'etanol-tag':
                            slotProps.data.energent ===
                        ...
                        {
                          'lozulje-tag':
                            slotProps.data.energent === 'Loživo ulje',
                        },
                        {
                          'lpg-tag':
                            slotProps.data.energent === ('LNG' || 'LPG') ||
                            slotProps.data.energent.charAt(0) === 'R',
                        },
                ...
                <Column
                  field="ukupnaEmisija"
                  header="Ukupne emisije CO2 t/god"
                />
                <template #footer>
                  <div class="flex justify-end font-bold w-full mt-4">
                    Ukupno:
        ...
        <section>
          ...
          <EnergyTable :editable="false" :data="izracuni" />
          <!-- <DataTable
            :value="opseg2Store.izracuni"
            show-gridlines
            :rows="5"
            data-key="id"
          >
            ...
            <Column header="Vrsta energije" field="energija">
              <template #body="slotProps">
                <template
                  v-if="slotProps.data.energija === 'Električna energija'"
                >
                  <font-awesome-icon
                    icon="bolt-lightning"
            ...
            <Column header="Neobnovljiva energija (kWh)" field="neobnovljivo">
              <template #body="slotProps">
                {{
                  slotProps.data.neobnovljivo !== null
            ...
            >
              <template #body="slotProps">
                {{
                  slotProps.data.obnovljivo !== null
            ...
            <template #footer>
              <div class="total-emissions">
                <span>Ukupno: </span>
                <strong>{{ formatNumber(totalEmissions) }}</strong> eCO<sub
                  >2</sub
                >
                t/god
              </div>
        ...
        <section>
          <div class="data-heading">
            ...
            <p>
              Prikaz odabranih kategorija emisija koje su izabrane kao
              relevantne iz Opsega 3
            </p>
          </div>
          <div class="kategorije">
            <Accordion
              v-if="filteredNodes.length && !loadingCategories"
              multiple
            >
              <AccordionPanel
                v-for="category in filteredNodes"
                :key="category.key"
                :value="category.key"
              >
                <AccordionHeader>
                ...
                <AccordionContent>
                  <p v-if="category && category.ukt_opis" class="tab-opis">
                    <span
                      v-for="(segment, index) in formatEndl(category.ukt_opis)"
                      :key="index"
                    >
                      {{ segment }}
                      <template
                        v-if="index < formatEndl(category.ukt_opis).length - 1"
                        ><br
                      /></template>
                    </span>
                  </p>
                      ...
                      >
                        ...
                        <AccordionContent>
                          <p v-if="child && child.ukt_opis" class="tab-opis">
                            <span
                              v-for="(segment, index) in formatEndl(
                                child.ukt_opis
                              )"
                              :key="index"
                            >
                              ...
                              <template
                                v-if="
                                  index < formatEndl(child.ukt_opis).length - 1
                                "
                                ><br
                              /></template>
                            </span>
            ...
            <span
              v-else
              style="display: flex; align-items: center; gap: 5px; opacity: 0.8"
            >
        ...
        <section>
          <div class="data-heading">
            <h2>Ukupna emisija (prema tržištu)</h2>
            <p>Ukupan prikaz stakleničkih plinova prema tržištu - Opseg 1 + Opseg 2</p>
          </div>
          <div class="ukupni-utrosak-o1o2">
            UKUPNO:
            <span class="emissions">
              {{ formatNumber(combinedEmissionsTrziste, 2) }}
            </span>
        ...
        <section>
          <div class="data-heading">
            <h2>Ukupna emisija (prema lokaciji)</h2>
            <p>Ukupan prikaz stakleničkih plinova prema lokaciji - Opseg 1 + Opseg 2</p>
          </div>
          <div class="ukupni-utrosak-o1o2">
            UKUPNO:
            <span class="emissions">
              {{ formatNumber(combinedEmissionsLokacija, 2) }}
            </span>
...
const energentiPotrosnja = computed(() => {
  ...
  return energenti.map((energent) => ({
    ...
    ukupnaPotrosnja:
      parseFloat(
        vehicleStore.ukupnaPotrosnjaGorivaZaEnergentIzSkupineEmisija(
          skupina,
          energent
        )
      ) || 0,
    mjernaJedinica:
      vehicleStore.mjernaJedinicaZaSkupinuEmisija(skupina) || "LL",
    ukupnaEmisija:
      (
        parseFloat(
          vehicleStore.ukupnaEmisijaGorivaZaEnergentIzSkupineEmisija(
            skupina,
            energent
          )
        ) / 1000
...
const totalLokEmissions = computed(() => opseg2Store.totalLokEmissions / 1000);
const totalCO2Emissions = computed(() => opseg2Store.totalCO2Emissions);

const combinedEmissionsTrziste = computed(() => {
...
const combinedEmissionsLokacija = computed(() => {
  // const opseg1 = parseFloat(calculateTotalEmissions().toFixed(2)) / 1000;
  const opseg1 = parseFloat(vehicleStore.ukupnaEmisija / 1000);
  const opseg2 = parseFloat(totalLokEmissions.value);
  const ukupno = opseg1 + opseg2;
  // console.log("opseg1: ", opseg1, "opseg2: ", opseg2, "ukupno: ", ukupno);
  return ukupno.toFixed(2);
});
...
const downloadReport = async (id) => {
  ...
  link.download = `ESP_${kespStore.getKespBrojIzracuna}_${formatDateString(
    datumOd.value
  )}_${formatDateString(datumDo.value)}.pdf`;
  ...
  vozila.value.forEach((vozilo) => {
    ...
    if (!emisijePoSkupini[kategorija]) {
      emisijePoSkupini[kategorija] =
        Number(vehicleStore.emisijaZaKategoriju(kategorija)).toFixed(2) / 1000;
    }
  });
...
.ukupni-utrosak-o1o2 {
  ...
  border: 1px solid var(--kesp-bg);
  border-left: 4px solid var(--kesp-primary);
