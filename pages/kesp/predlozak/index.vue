      ...
      <div class="main-content">
        ...
        <section id="sva-vozila">
          <div>
            ...
            <p>
              Emisije iz izvora stakleničkih plinova u vlasništvu ili pod
              kontrolom organizacije:
            </p>
            <ul>
              <li>
                Izravne emisije iz pokretnih energetskih izvora (vozila,
                pokretni strojevi i oprema)
              </li>
              <li>
                Izravne emisije iz nepokretnih energetskih izvora
                <ul>
                  <li>
                    Točkasti: postrojenja, tehnološki procesi, industrijski
                    pogoni, uređaji, građevine i sl.
                  </li>
                  <li>
                    Difuzni: uređaji, određene aktivnosti, površine i druga
                    mjesta
                  </li>
                </ul>
              </li>
              <li>
                Izravne fugitivne emisije (emisija iz rashladnih uređaja i
                postrojenja te protupožarnih aparata)
              </li>
            </ul>
          </div>
          <div class="datatable-container">
            ...
            <DataTable
              v-model:selection="selectedVozilo"
              row-group-mode="subheader"
              group-rows-by="uge_naziv"
            ...
            >
              ...
              <template #groupfooter="slotProps">
                ...
                >
                  ...
                  <strong style="margin-left: 5px">
                    {{
                      formatNumber(
                        vehicleStore.emisijaZaKategoriju(
                          slotProps.data.uge_naziv,
                          2
                        )
                      )
              ...
              <Column field="gorivo" header="Energent" sortable>
                <template #body="slotProps">
                  <Tag
                    :value="slotProps.data.uvg_knaziv"
                    :class="[
                      {
                        'etanol-tag':
                          slotProps.data.uvg_knaziv ===
                            ('Etanol', 'Prirodni plin') ||
                          (odabranaSkupina?.uge_naziv ==
                            'Staklenički plinovi (GWP)' &&
                      ...
                      },
                      {
                        'jet-tag':
                          slotProps.data.uvg_knaziv ===
                          ('Jet Fuel', 'Gorivo za mlazne motore'),
                      },
                      {
                        'lozulje-tag':
                          slotProps.data.uvg_knaziv === 'Loživo ulje',
                      },
                      {
                        'lpg-tag':
                          slotProps.data.uvg_knaziv === ('LNG' || 'LPG') ||
                          slotProps.data.uvg_knaziv.charAt(0) === 'R',
                      },
              ...
              <Column field="emisije" header="Emisija eCO2/kg" sortable>
                <template #body="slotProps">
                  <span>{{
                    formatNumber(parseFloat(slotProps.data.usi_emisija, 2))
                  }}</span>
                </template>
              </Column>
              <template #footer>
                <div class="total-emissions">
                  <div>
                    Ukupno:
                    <strong>{{ formatNumber(ukupnaEmisija, 2) }}</strong>
                    eCO<sub>2</sub>/kg
                  </div>
                  <!--.toFixed(2)-->
                </div>
            ...
            <Dialog
              v-model:visible="voziloDialogVisible"
              ...
              <form class="p-fluid" @submit.prevent="saveVozilo">
                ...
                <div class="field">
                  ...
                  <Select
                    ...
                    id="vrstaVozila"
                    v-model="odabranaVrstaEmsiije"
                    :options="vrsteEmisija"
                    option-label="uvv_naziv"
                    :placeholder="
                ...
                <div class="field">
                  ...
                  <Select
                    id="gorivo"
                    v-model="odabraniEnergent"
                    :options="vrsteEnergenata"
            ...
            <!-- GWP Dialog -->
            <Dialog
              v-model:visible="gwpDialogVisible"
              ...
              >
                ...
                <Column field="uir_emisija" header="Emisija eCO2/kg">
                  <template #body="{ data, field }">
                    <strong
                      style="text-decoration: none; color: var(--kesp-primary)"
            ...
            >
              <div class="flex items-center gap-4">
                <template v-if="selectedVozilo">
                  ...
                  <br />
                  Emisije CO<sub>2</sub>/kg:
                  <b>{{ Number(selectedVozilo.usi_emisija).toFixed(2) }}</b>
                </template>
              </div>
              <template #footer>
                <button
          ...
          <GwpIzracuni style="margin-top: 26px" />
        </section>
      </div>
...
import GwpIzracuni from "~/components/kesp/GwpIzracuni.vue";
import {
...
const gwpPredlosci = computed(() => kespStore.gwpPredlosci);
...
const rowSchema = ref(
  zodResolver(
    z.object({
      uir_brjed: z
        .number({ message: "Broj jedinica mora biti broj" })
        .min(1, { message: "Broj jedinica je obavezan i mora biti veći od 0" }),
      ugr_naziv: z
        .object({
          ugr_naziv: z
            .string()
            .min(1, { message: "Vrsta izračuna je obvezna" }),
        })
        .refine((obj) => obj && obj.tvz_naziv, {
          message: "Vrsta izračuna je obvezna",
        }),
      uir_kapacitet: z
        .number({ message: "Količina rashladnog sredstva mora biti broj" })
        .min(
          1,
          "Količina rashladnog sredstva je obavezna i mora biti veća od 0"
        ),
      uir_ugradnja: z.string().min(1, { message: "Polje je obavezno" }),
      uir_rad: z.string().min(1, { message: "Polje je obavezno" }),
      uir_kraj: z.string().min(1, { message: "Polje je obavezno" }),
      // Ostala polja možete dodati ako trebaju validaciju
    })
  )
);
...
//     try {
//     // Pokušajte validirati red pomoću Zod sheme
//     rowSchema.parse(rowData);
//     // Ako je validacija uspješna, nastavite s logikom spremanja
//     console.log("Validacija prošla, red je spreman za spremanje.");
//     // Ovdje dodajte kod za spremanje ili daljnju obradu
//   } catch (error) {
//     // Ako validacija ne uspije, ispišite pogreške ili obavijestite korisnika
//     console.error("Validacijske pogreške:", error.errors);
//     // Možete ovdje postaviti odgovarajuće state vrijednosti kako bi prikazali poruke korisniku
//   }
// }
...
const isGwpDataValid = (data) => {
  ...
  if (
    data?.uir_brjed <= 0 ||
    !data?.ugr_naziv ||
    data?.uir_kapacitet <= 0 ||
    !data?.uir_ugradnja ||
    !data?.uir_rad ||
    !data?.uir_kraj
  ) {
    invalidRows.value.push({ message: "Sva polja su obavezna." });
    valid = false;
  }
  ...
  if (
    ...
    invalidRows.value.push({
      message: "Sva tri Da/Ne polja ne mogu imati isti odgovor: 'Ne'.",
    });
    valid = false;
  }
...
/**
 * Function called when the user clicks the "Save" button on the GWP izračun dialog.
 * It validates the input data, updates the UI and database accordingly, and shows a success or error message.
 * @param {Object} event - The event object passed by the PrimeVue DataTable component.
 * @param {Object} event.data - The row data being edited.
 * @param {Number} event.index - The index of the row being edited.
 */
async function onGwpIzracunSave(event) {
  let { newData, index } = event;
  ...
  if (isGwpDataValid(newData)) {
    const getUsiId = async () => {
      const foundVehicle = vozila.value.find(
        (item) => item?.usi_uvg_id === odabraniEnergent.value?.uvg_id
      );
    ...
    const updateNewData = async () => {
      ...
      const ugrGroup = vrsteRashUredjaja.value.find(
        (item) => item?.ugr_naziv === newData?.ugr_naziv
      );
...
onMounted(async () => {
  try {
    ...
    // gwpIzracuni.value = mapByMatch(gwpIzracuni.value, vrsteRashUredjaja.value, "ugr_naziv");
    kespStore.fetchCoolingCalculation(kespId.value);
    // console.log("gwpStorePredlosci: ", kespStore.gwpPredlosci)
...
watch(odabranaSkupina, async () => {
  ...
  try {
    vrsteEmisija.value = await getVehiclesForEmmisionGroups(
      parseInt(odabranaSkupina.value.uge_id)
    );
  ...
  if (!parseInt(odabranaSkupina.value?.br_vozila)) {
    try {
      vrsteEnergenata.value = await getFuelTypes(
        parseInt(odabranaSkupina.value.uge_id)
      );
...
watch(odabranaVrstaEmsiije, async () => {
  ...
  if (parseInt(odabranaSkupina.value?.br_vozila)) {
    try {
      vrsteEnergenata.value = await getFuelTypes(
        parseInt(odabranaSkupina.value.uge_id),
        parseInt(odabranaVrstaEmsiije.value.uvv_id)
      );
      // console.log("Evo vrste goriva: ", vrsteEnergenata.value);
    } catch (error) {
      // console.error('Error fetching fuels:', error);
...
watch(odabraniEnergent, async () => {
  ...
  if (odabraniEnergent.value?.uvg_oznaka) {
    gwpIzracuni.value = [];
    const foundGwp = vozila.value.find(
      (item) => item?.uvg_naziv === odabraniEnergent.value?.uvg_naziv
    );
...
async function saveGwpIzracun() {
  try {
    await vehicleStore.fetchVehicles(kespId.value);
    await kespStore.fetchCoolingCalculation(kespId.value);
  } catch (error) {
...
const chartData = computed(() => {
  ...
  vozila.value.forEach((vozilo) => {
    ...
    if (!emisijePoSkupini[kategorija]) {
      emisijePoSkupini[kategorija] =
        vehicleStore.emisijaZaKategoriju(kategorija);
    }
  });
...
const polarChartData = computed(() => {
  ...
  vozila.value.forEach((vozilo) => {
    ...
    if (!potrosnjaPoSkupini[kategorija]) {
      potrosnjaPoSkupini[kategorija] =
        vehicleStore.potrosnjaGorivaZaKategoriju(kategorija);
    }
  });
...
const showSaveSuccess = (isEdit, vrsta) => {
  toast.add({
    severity: "success",
    summary: isEdit ? "Uspješno uređeno" : "Uspješno dodano",
    detail: `Vrsta: ${vrsta}`,
    life: 3000,
  });
};
...
const showDeleteVozilo = () => {
  toast.add({
    severity: "success",
    summary: "Uspješno obrisano",
    detail: `Obrisano vozilo`,
    life: 3000,
  });
};
...
const showError = () => {
  toast.add({
    severity: "error",
    summary: "Došlo je do greške",
    detail: `Nije uspješno izvršena radnja`,
    life: 3000,
  });
};
...
const checkSameEmisije = (skupinaId, vrsta, gorivoId) => {
  return (
    vozila.value.find(
      (vozilo) =>
        vozilo.usi_uge_id == skupinaId &&
        vozilo.uvv_naziv === vrsta &&
        vozilo.usi_uvg_id == gorivoId
    )?.usi_id || null
  );
};
...
const saveVozilo = async () => {
  ...
  const data = {
    ...
    veichleTypeId:
      typeof odabranaVrstaEmsiije.value === "object"
...
const deleteVozilo = async () => {
  try {
    ...
    if (status === 200) {
      const index = vozila.value.findIndex(
        (v) => v?.usi_id === selectedVozilo.value?.usi_id
      );
      if (index !== -1) {
...
@media (max-width: 1400px) {
  /* Sakrij stats-content pri manjim ekranima */
  .stats-content {
    display: none;
  }

  /* Prilagodi grid layout na jednu kolonu */
  main {
    grid-template-columns: 1fr;
  }

  /* Osiguraj da se main-content prilagođava */
  .main-content {
    max-width: 100%;
  }
}
