      ...
      <div class="main-contentt">
        <section class="title">
          ...
          <div class="data-heading" style="margin-top: 23px">
            ...
            <p>
              Ostale emisije koje nastaju u vrijednosnom lancu organizacije te
              su posljedica poslovne aktivnosti organizacije ali proizlaze iz
              izvora koje organizacije ne posjeduje ili ne kontrolira.
            </p>
          </div>
        </section>
        <section class="content-section">
          ...
          <section class="sidebar">
            ...
            <div class="sidebar-content">
              <Tree
                ...
                class="tree"
              >
                <!-- @node-select="showTabs($event.key)"-->
                <template #default="slotProps">
                  <div
                    v-tooltip.top="slotProps.node.ukt_naziv"
          ...
          <section class="view">
            <Tabs
              v-if="Object.keys(selectedKey).length > 0"
              ...
              <TabPanels>
                ...
                >
                  <div class="tab-panel">
                    ...
                    <div
                      v-if="tab.title === 'Opis kategorije'"
                      class="relevant"
                    >
                      <!--!tab.children.length && -->
                      <Checkbox
                        v-model="checked"
                        :value="parseInt(tab.key)"
                        :input-id="'checkbox-' + tab.key"
                        @change="onRelevantChange(tab.key)"
                      />
...
watch(selectedKey, (newSelectedKey) => {
  ...
  const selectedNodeKey = Object.keys(newSelectedKey).find(
    (key) => newSelectedKey[key] === true
  );
        ...
        tabs.value = [
          {
            ...
            content: selectedNode.ukt_opis || "Nema opisa kategorije",
            children: selectedNode.children,
          },
          {
            key: selectedNode.key,
            title: "Podaci za prikupljanje",
            content: selectedNode.ukt_podaci || "Nema podataka za prikupljanje",
            children: selectedNode.children,
          },
          {
            key: selectedNode.key,
            title: "Način prikupljanja podataka",
            content:
              selectedNode.ukt_nacin || "Nema načina prikupljanja podataka",
            children: selectedNode.children,
          },
          {
            key: selectedNode.key,
            title: "Dinamika",
            content: selectedNode.ukt_dinamika || "Nema dinamike",
            children: selectedNode.children,
          },
          {
            key: selectedNode.key,
            title: "Minimalna granica",
            content: selectedNode.ukt_mingran || "Nema minimalne granice",
            children: selectedNode.children,
          },
        ];
      } else {
...
const showSuccess = () => {
  toast.add({
    ...
    detail: "Promjenjena relevantnost je spremljena",
    life: 3000,
  });
};
    ...
    tabs.value = [
      {
        key: node.key,
        title: "Opis kategorije",
        content: node.ukt_opis || "Nema opisa kategorije",
        children: node.children,
      },
      {
        key: node.key,
        title: "Podaci za prikupljanje",
        content: node.ukt_podaci || "Nema podataka za prikupljanje",
        children: node.children,
      },
      {
        key: node.key,
        title: "Način prikupljanja podataka",
        content: node.ukt_nacin || "Nema načina prikupljanja podataka",
        children: node.children,
      },
      {
        key: node.key,
        title: "Dinamika",
        content: node.ukt_dinamika || "Nema dinamike",
        children: node.children,
      },
      {
        key: node.key,
        title: "Minimalna granica",
        content: node.ukt_mingran || "Nema minimalne granice",
        children: node.children,
      },
    ];
  } else {
    ...
    if (parentNode) {
      ...
      if (allChildrenUnchecked) {
        ...
        checked.value = checked.value.filter(
          (item) => item !== parseInt(parentNode.key)
        );
...
.p-tab-active {
  background: none !important;
  border-color: var(--kesp-primary) !important;
  border-radius: 0;
...
:deep(.p-tablist-active-bar) {
  background: var(--kesp-primary) !important;
}
